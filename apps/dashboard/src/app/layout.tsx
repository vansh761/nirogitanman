...
export default async function DashboardLayout({
  ...
  const role =
    (session?.user as { role?: Role } | undefined)?.role ?? "FREE_USER";
  ...
  try {
    if (session?.user?.id) {
      notifications = await getNotificationsForUser(session.user.id as string);
    }
  } catch {
    ...
    <Providers>
      <div className="flex min-h-screen">
        ...
        <div className="flex-1 flex flex-col">
          ...
          <main className="flex-1 p-6 bg-forest/[0.015] dark:bg-mint/[0.015]">
            {children}
          </main>
        </div>
      </div>
